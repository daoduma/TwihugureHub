"use client";
// app/(dashboard)/farmer/profile/page.tsx

import { useState, useEffect } from "react";
import { useSession } from "next-auth/react";
import { User, Mail, Globe, Sprout, Loader2, CheckCircle } from "lucide-react";
import { useTranslation, useLanguageSwitcher } from "@/lib/useTranslation";
...
export default function FarmerProfilePage() {
  ...
  async function saveProfile(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        await update({ name, preferredLanguage: language });
        // Sync i18n immediately so all UI strings flip to the new language
        await changeLanguage(language as "en" | "fr" | "rw");
  ...
  return (
    <div className="animate-fade-in mx-auto max-w-2xl space-y-6">
      ...
      <div className="card flex items-center gap-4 p-5">
        <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-green-600 text-white shadow-lg shadow-green-900/20">
          <Sprout size={28} />
        </div>
        <div>
