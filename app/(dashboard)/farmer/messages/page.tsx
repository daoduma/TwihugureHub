...
export default function FarmerMessagesPage() {
  ...
  // Mark a single message as read when it is opened
  function openMessage(msg: Message) {
    ...
    if (!msg.readAt) {
      ...
      // Persist via PATCH (only this message)
      fetch("/api/farmer/messages", {
        method: "PATCH",
  ...
  const ROLE_LABELS: Record<string, string> = {
    MBAZA_STAFF: t("farmer.messages.roleMbaza" as never) || "Mbaza Staff",
    ADMIN:       t("farmer.messages.roleAdmin"  as never) || "Admin",
    TRAINER:     t("farmer.messages.roleTrainer"as never) || "Trainer",
  };
    ...
    <div className="animate-fade-in space-y-5">
      <div>
        <h1 className="text-2xl font-bold text-brand-900" style={{ fontFamily: "var(--font-display)" }}>
          {t("farmer.messages.title" as never) || "My Messages"} ✉️
        </h1>
        <p className="text-sm text-gray-500 mt-1">
          {loading
            ? t("common.loading" as never)
            : t("farmer.messages.summary" as never, {
                count: messages.length,
                unread,
              }) || `${messages.length} ${t("farmer.messages.messages" as never) || "messages"} · ${unread} ${t("farmer.messages.unread" as never) || "unread"}`}
        </p>
      </div>
          ...
          {!loading && messages.length === 0 && (
            <div className="p-10 text-center">
              ...
              <p className="text-sm text-gray-400">
                {t("farmer.messages.empty" as never) || "No messages yet"}
              </p>
            </div>
          )}
          ...
          {!selected ? (
            <div className="flex items-center justify-center h-full min-h-[200px] text-gray-400">
              <div className="text-center">
                <p className="text-3xl mb-2">📩</p>
                <p className="text-sm">
                  {t("farmer.messages.selectPrompt" as never) || "Select a message to read"}
                </p>
              </div>
            </div>
          ) : (
            <div className="space-y-4">
              <div className="border-b border-gray-100 pb-4">
                ...
                <div className="flex flex-wrap items-center gap-3 mt-2 text-sm text-gray-500">
                  <span>
                    {t("farmer.messages.from" as never) || "From"}:{" "}
                    <strong>{selected.sender.name}</strong>{" "}
                    ({ROLE_LABELS[selected.sender.role] ?? selected.sender.role})
                  </span>
                  <span>{new Date(selected.sentAt).toLocaleString()}</span>
                  <span className="rounded-full bg-gray-100 text-gray-500 px-2 py-0.5 text-xs uppercase">
                    {selected.language}
                  </span>
                  {selected.readAt && (
                    <span className="rounded-full bg-green-100 text-green-600 px-2 py-0.5 text-xs">
                      {t("farmer.messages.read" as never) || "Read"}
                    </span>
                  )}
                </div>
              </div>
