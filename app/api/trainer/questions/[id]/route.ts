...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    console.error("[trainer/questions/[id]] PUT error:", err);
    return NextResponse.json(
      { success: false, error: err instanceof Error ? err.message : "Failed to update question" },
      { status: 500 }
    );
  }
}
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    console.error("[trainer/questions/[id]] DELETE error:", err);
    return NextResponse.json(
      { success: false, error: err instanceof Error ? err.message : "Failed to delete question" },
      { status: 500 }
    );
  }
}
